...
// Eigener Variablen-Name (NICHT __dirname), weil Netlifys Function-Bundler
// __dirname / __filename als auto-injected Globals nochmal deklariert und
// dann eine "Identifier already declared"-SyntaxError beim Cold-Start wirft.
const LIB_DIR = dirname(fileURLToPath(import.meta.url));
const TEMPLATE_PATH = join(LIB_DIR, '..', 'assets', 'dkv-einsatzbogen-vorlage.pdf');
...
export async function generateEinsatzbogenPdf({ referee = {}, year, entries = [] }) {
  ...
  drawLeft(page1, helv,     referee.verband ?? 'KVN',               HEADER.verband);
  drawLeft(page1, helv,     referee.phone ?? '',                    HEADER.telefon);
  drawLeft(page1, helv,     referee.ausweisNr ?? '',                HEADER.ausweisNr);
